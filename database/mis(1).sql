...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Aug 12, 2025 at 10:20 AM
...
-- Procedures
--
DROP PROCEDURE IF EXISTS `academic`$$
CREATE DEFINER=`misuser`@`%` PROCEDURE `academic` ()  NO SQL BEGIN
...
DROP PROCEDURE IF EXISTS `chat_group`$$
...
DROP PROCEDURE IF EXISTS `feedback_done`$$
CREATE DEFINER=`misuser`@`%` PROCEDURE `feedback_done` (IN `suid` INT(11), IN `stid` VARCHAR(20), IN `sdate` DATE, IN `stime` TIME)  NO SQL BEGIN
INSERT into feedback_done(survey_id,student_id,s_date,s_time) 
VALUES(suid,stid,sdate,stime);
END$$

DROP PROCEDURE IF EXISTS `feedback_survey`$$
CREATE DEFINER=`misuser`@`%` PROCEDURE `feedback_survey` (IN `suid` INT(11), IN `did` VARCHAR(6), IN `ayear` VARCHAR(11), IN `cid` VARCHAR(11), IN `mid` VARCHAR(11), IN `sid` VARCHAR(64), IN `sdate` DATE, IN `edate` DATE)  NO SQL BEGIN
...
DROP PROCEDURE IF EXISTS `food`$$
CREATE DEFINER=`misuser`@`%` PROCEDURE `food` ()   BEGIN
    SELECT *  FROM food;
END$$

DROP PROCEDURE IF EXISTS `getCourse`$$
CREATE DEFINER=`misuser`@`%` PROCEDURE `getCourse` ()  NO SQL BEGIN
SELECT * FROM `course`;
END$$
...
DROP PROCEDURE IF EXISTS `manage_final_place`$$
...
CREATE DEFINER=`misuser`@`%` PROCEDURE `notice_insert` (IN `eid` INT(11), IN `ename` VARCHAR(255), IN `evenue` VARCHAR(255), IN `edate` DATE, IN `echiefguest` VARCHAR(255))  NO SQL BEGIN
...
DROP PROCEDURE IF EXISTS `notice_result`$$
CREATE DEFINER=`misuser`@`%` PROCEDURE `notice_result` (IN `rid` INT(11), IN `did` VARCHAR(6), IN `ayear` VARCHAR(11), IN `cid` VARCHAR(11), IN `mid` VARCHAR(11), IN `uplod` VARCHAR(30))  NO SQL BEGIN
insert INTO
notice_result(result_id,department_id,academic_year,course_id,module_id,upload)
VALUES(rid,did,ayear,cid,mid,uplod);
END$$

DROP PROCEDURE IF EXISTS `notice_update`$$
CREATE DEFINER=`misuser`@`%` PROCEDURE `notice_update` (IN `e_name` VARCHAR(255), IN `e_venue` VARCHAR(255), IN `e_chiefguest` VARCHAR(30), IN `e_id` INT(11), IN `e_date` DATE, IN `e_comment` VARCHAR(255))  NO SQL BEGIN 
...
DROP PROCEDURE IF EXISTS `notionalhours`$$
...
DROP PROCEDURE IF EXISTS `request_onpeak`$$
...
DROP PROCEDURE IF EXISTS `selectmodules`$$
...
DROP PROCEDURE IF EXISTS `show_message`$$
...
-- Functions
--
DROP FUNCTION IF EXISTS `course_update`$$
CREATE DEFINER=`misuser`@`%` FUNCTION `course_update` (`cid` VARCHAR(11), `cname` VARCHAR(255), `cnvq` ENUM('3','4','5','6','BRI'), `cojt` INT(2), `ctrai` INT(2), `did` VARCHAR(6)) RETURNS VARCHAR(255) CHARSET latin1 NO SQL BEGIN
...
DROP FUNCTION IF EXISTS `event_update`$$
CREATE DEFINER=`misuser`@`%` FUNCTION `event_update` (`eid` INT(11), `ename` VARCHAR(255), `evenue` VARCHAR(255), `edate` DATE, `echiefguest` VARCHAR(30), `ecomment` TEXT, `etime` TIME, `eurl` VARCHAR(30), `estatus` VARCHAR(20)) RETURNS VARCHAR(255) CHARSET latin1 DETERMINISTIC NO SQL BEGIN
...
DROP FUNCTION IF EXISTS `feedback_survey`$$
CREATE DEFINER=`misuser`@`%` FUNCTION `feedback_survey` (`suid` INT(11), `did` VARCHAR(6), `ayear` VARCHAR(11), `cid` VARCHAR(11), `mid` VARCHAR(11), `sid` VARCHAR(64), `sdate` DATE, `edate` DATE) RETURNS INT DETERMINISTIC NO SQL BEGIN INSERT INTO feedback_survey VALUES(suid,did,ayear,cid,mid,sid,sdate,edate);
RETURN 1;
END$$

DROP FUNCTION IF EXISTS `notice_event`$$
CREATE DEFINER=`misuser`@`%` FUNCTION `notice_event` (`eid` INT(11), `ename` VARCHAR(255), `evenue` VARCHAR(255), `edate` DATE, `echiefguest` VARCHAR(255), `ecomment` TEXT, `etime` TIME, `eurl` VARCHAR(30), `estatus` VARCHAR(20)) RETURNS INT DETERMINISTIC NO SQL BEGIN
INSERT INTO notice_event VALUES(eid,ename,evenue,edate,echiefguest,ecomment,etime,eurl,estatus);
RETURN 1;
END$$

DROP FUNCTION IF EXISTS `return_status`$$
CREATE DEFINER=`misuser`@`%` FUNCTION `return_status` (`returned_date` DATE) RETURNS VARCHAR(10) CHARSET latin1 NO SQL BEGIN
...
CREATE TABLE IF NOT EXISTS `assessments` (
  ...
  KEY `academy_year` (`academic_year`),
  KEY `course_name` (`course_id`),
  KEY `assessment_type_id_2` (`assessment_type_id`),
...
DROP TABLE IF EXISTS `assessments_marks`;
CREATE TABLE IF NOT EXISTS `assessments_marks` (
  `assessment_marks_id` int NOT NULL AUTO_INCREMENT,
  `assessment_id` int NOT NULL,
  `student_id` varchar(25) CHARACTER SET utf8mb3 COLLATE utf8mb3_unicode_ci NOT NULL,
  ...
  PRIMARY KEY (`assessment_marks_id`,`student_id`),
...
DROP TABLE IF EXISTS `assessments_type`;
CREATE TABLE IF NOT EXISTS `assessments_type` (
...
CREATE TABLE IF NOT EXISTS `books` (
  `numbering` int NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `chat_group_member` (
  ...
  `chat_group_member` varchar(64) CHARACTER SET utf8mb3 COLLATE utf8mb3_unicode_ci NOT NULL,
...
CREATE TABLE IF NOT EXISTS `chat_group_message` (
  ...
  `chat_group_sender` varchar(64) CHARACTER SET utf8mb3 COLLATE utf8mb3_unicode_ci NOT NULL,
  `chat_group_reciver_group_id` int NOT NULL,
  PRIMARY KEY (`message_id`),
  KEY `chat_group_reciver_group_id` (`chat_group_reciver_group_id`),
  KEY `chat_group_sender` (`chat_group_sender`)
...
CREATE TABLE IF NOT EXISTS `course` (
  ...
  `course_ojt_duration` int NOT NULL,
  `course_institute_training` int NOT NULL,
...
DROP TABLE IF EXISTS `course_delete_tracking`;
CREATE TABLE IF NOT EXISTS `course_delete_tracking` (
...
DROP TABLE IF EXISTS `delete_feedback_survey`;
CREATE TABLE IF NOT EXISTS `delete_feedback_survey` (
...
CREATE TABLE IF NOT EXISTS `donation` (
  ...
  UNIQUE KEY `donation_id_2` (`donation_id`),
  UNIQUE KEY `donation_id_3` (`donation_id`),
  UNIQUE KEY `donation_id_4` (`donation_id`)
...
DROP TABLE IF EXISTS `event_delete`;
...
DROP TABLE IF EXISTS `feedback_done`;
CREATE TABLE IF NOT EXISTS `feedback_done` (
  `survey_id` int NOT NULL,
  ...
  KEY `student_id` (`student_id`),
  KEY `idx_survey_student` (`survey_id`,`student_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE IF NOT EXISTS `feedback_survey` (
  ...
  PRIMARY KEY (`survey_id`),
  KEY `staff_id` (`staff_id`),
  KEY `feedback_survey_ibfk_3` (`course_id`),
  KEY `feedback_survey_ibfk_4` (`module_id`),
  KEY `feedback_survey_ibfk_1` (`department_id`),
  KEY `feedback_survey_ibfk_2` (`academic_year`),
  KEY `idx_course_module_dates` (`course_id`,`module_id`,`start_date`,`end_date`)
...
--
DROP TRIGGER IF EXISTS `delete_feedback_survey`;
DELIMITER $$
...
CREATE TABLE IF NOT EXISTS `food_order` (
  ...
  KEY `user_id` (`food_order_user_name`)
...
CREATE TABLE IF NOT EXISTS `food_order_details` (
  `food_order_details_food_order_id` int NOT NULL,
  ...
  PRIMARY KEY (`food_order_details_food_order_id`,`food_order_details_food_id`),
  KEY `food_order_id` (`food_order_details_food_order_id`),
  KEY `food_id` (`food_order_details_food_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
DROP TABLE IF EXISTS `hostel_student_details`;
CREATE TABLE IF NOT EXISTS `hostel_student_details` (
  `hosttler_id` int NOT NULL AUTO_INCREMENT,
  `student_id` varchar(20) CHARACTER SET utf8mb3 COLLATE utf8mb3_unicode_ci NOT NULL,
...
CREATE TABLE IF NOT EXISTS `inventory` (
  ...
  UNIQUE KEY `department_id` (`inventory_department_id`),
  KEY `itemid` (`item_id`)
...
DROP TABLE IF EXISTS `inventory_item_supplier`;
CREATE TABLE IF NOT EXISTS `inventory_item_supplier` (
  `supplier_id` varchar(30) NOT NULL,
...
--
DROP TRIGGER IF EXISTS `copy_status_update`;
DELIMITER $$
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `issued_books_deleted`;
DELIMITER $$
...
DROP TABLE IF EXISTS `issued_books_deleted`;
CREATE TABLE IF NOT EXISTS `issued_books_deleted` (
...
DROP TABLE IF EXISTS `manage_final_place`;
CREATE TABLE IF NOT EXISTS `manage_final_place` (
...
CREATE TABLE IF NOT EXISTS `module` (
  ...
  `module_learning_hours` int NOT NULL,
  ...
  `module_lecture_hours` int NOT NULL,
  `module_practical_hours` int NOT NULL,
  `module_self_study_hours` int NOT NULL,
  `course_id` varchar(11) NOT NULL,
  PRIMARY KEY (`module_id`,`course_id`),
  KEY `course_id` (`course_id`),
  KEY `idx_course_module` (`course_id`,`module_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
DROP TABLE IF EXISTS `notice_event_stutas`;
CREATE TABLE IF NOT EXISTS `notice_event_stutas` (
...
DROP TABLE IF EXISTS `notice_result`;
CREATE TABLE IF NOT EXISTS `notice_result` (
...
DROP TABLE IF EXISTS `off_peak`;
CREATE TABLE IF NOT EXISTS `off_peak` (
...
-- Triggers `ojt`
--
DROP TRIGGER IF EXISTS `USER_CREATION_OJT`;
DELIMITER $$
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `USER_DELETE_OJT`;
DELIMITER $$
...
DROP TABLE IF EXISTS `ojt_delete_details`;
CREATE TABLE IF NOT EXISTS `ojt_delete_details` (
...
DROP TABLE IF EXISTS `onpeak_delete_details`;
CREATE TABLE IF NOT EXISTS `onpeak_delete_details` (
  `dbuser` varchar(50) NOT NULL,
...
DROP TABLE IF EXISTS `onpeak_request`;
CREATE TABLE IF NOT EXISTS `onpeak_request` (
  `id` int NOT NULL AUTO_INCREMENT,
  `student_id` varchar(20) CHARACTER SET utf8mb3 COLLATE utf8mb3_unicode_ci NOT NULL,
  `department_id` varchar(6) NOT NULL,
  `contact_no` int NOT NULL,
  ...
  PRIMARY KEY (`id`,`exit_date`),
  KEY `student_id` (`student_id`),
  KEY `onpeak_request_ibfk_1` (`department_id`)
...
--
DROP TRIGGER IF EXISTS `USER_DELETE_ONPEAK`;
DELIMITER $$
...
CREATE TABLE IF NOT EXISTS `payment` (
  ...
  PRIMARY KEY (`payment_reason`,`payment_type`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE IF NOT EXISTS `pays` (
  ...
  `pays_qty` int NOT NULL,
  `pays_date` date NOT NULL,
  `pays_department` varchar(20) NOT NULL,
  PRIMARY KEY (`pays_id`),
  KEY `payment_id` (`payment_reason`),
  KEY `student_id` (`student_id`),
  KEY `payment_id_2` (`payment_reason`),
  KEY `payment_type` (`payment_type`),
  KEY `payment_type_2` (`payment_type`)
...
--
-- Table structure for table `precreated_users`
--

DROP TABLE IF EXISTS `precreated_users`;
...
--
-- Dumping data for table `precreated_users`
--

INSERT INTO `precreated_users` (`id`, `username`, `email`, `full_name`, `department`, `position`, `phone_no`, `created_at`, `is_used`, `used_at`) VALUES
(39, 'new', '', '', '', 'student', '0778870135', '2025-08-12 10:19:40', 1, '2025-08-12 15:49:47');
...
CREATE TABLE IF NOT EXISTS `staff` (
  ...
  `staff_email` varchar(254) CHARACTER SET utf8mb3 COLLATE utf8mb3_unicode_ci NOT NULL,
  `staff_pno` int NOT NULL,
  ...
  UNIQUE KEY `staff_nic` (`staff_nic`),
  UNIQUE KEY `staff_epf_no` (`staff_epf`),
...
-- Triggers `staff`
--
DROP TRIGGER IF EXISTS `USER_CREATION_STAFF`;
DELIMITER $$
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `USER_NAME_UPDATE_STAFF`;
DELIMITER $$
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `USER_POSITION_UPDATE_STAFF`;
DELIMITER $$
...
DROP TABLE IF EXISTS `staff_module_enrollment`;
CREATE TABLE IF NOT EXISTS `staff_module_enrollment` (
  `staff_module_enrollment_id` int NOT NULL AUTO_INCREMENT,
  `staff_id` varchar(64) CHARACTER SET utf8mb3 COLLATE utf8mb3_unicode_ci NOT NULL,
  `course_id` varchar(11) NOT NULL,
  `module_id` varchar(11) NOT NULL,
...
DROP TABLE IF EXISTS `staff_position_type`;
CREATE TABLE IF NOT EXISTS `staff_position_type` (
  `staff_position_type_id` varchar(11) NOT NULL,
  `staff_position_type_name` varchar(64) NOT NULL,
  `staff_position` tinyint NOT NULL,
  PRIMARY KEY (`staff_position_type_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE IF NOT EXISTS `student` (
  ...
  `student_email` varchar(254) CHARACTER SET utf8mb3 COLLATE utf8mb3_unicode_ci NOT NULL,
...
-- Triggers `student`
--
DROP TRIGGER IF EXISTS `USER_CREATION_STUDENT`;
DELIMITER $$
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `USER_DELETE_STUDENT`;
DELIMITER $$
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `USER_EMAIL_UPDATE_SUDENT`;
DELIMITER $$
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `USER_NAME_UPDATE_STUDENT`;
DELIMITER $$
...
CREATE TABLE IF NOT EXISTS `student_enroll` (
  ...
  KEY `course_id` (`course_id`),
  KEY `enroll_student_id` (`student_id`),
  KEY `student_enroll_ibfk_3` (`academic_year`),
...
CREATE TABLE IF NOT EXISTS `student_qualification` (
  `qualification_student_id` varchar(20) CHARACTER SET utf8mb3 COLLATE utf8mb3_unicode_ci NOT NULL,
  ...
  PRIMARY KEY (`qualification_student_id`,`qualification_index_no`,`qualification_description`),
  KEY `qualification_student_id` (`qualification_student_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
--
-- Stand-in structure for view `student_surveys`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `student_surveys`;
...
(379, 'ICT', '4IT', 'M01', '2020/2021', 'achchuthan', 'Monday', 'P1', 'LAP-01', '2025-06-01', '2025-07-31'),
(380, 'ICT', '5IT', 'M01', '2020/2021', 'romiyal', 'Monday', 'P2', 'LAP-01', '2025-07-01', '2025-08-30');
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `user_table` enum('student','staff') CHARACTER SET utf8mb3 COLLATE utf8mb3_unicode_ci NOT NULL DEFAULT 'student',
  `staff_position_type_id` varchar(11) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL,
  ...
  `user_email` varchar(254) CHARACTER SET utf8mb3 COLLATE utf8mb3_unicode_ci NOT NULL COMMENT 'user''s email, unique',
  ...
  KEY `staff_position_type_id` (`staff_position_type_id`)
) ENGINE=InnoDB AUTO_INCREMENT=96 DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci COMMENT='user data';
...
(1, 'staff', 'ADM', NULL, 'keerththi', '$2y$10$gFoU2RiUUhAy8/tC7wKUHuF7D7RQdK3RAVR2Xr//tn06VY6iP.ZNW', 'achchuthan@slgti.com', 1, 0, '6d7ccfe2be45092b92ea24e7fc89beac0b06fea13deb4bc4ee92116e4f7d463e', NULL, NULL, NULL, 0, NULL, NULL, '51dd5913fe97220bd88932b559f159b89c27200c', 1754901288),
(2, 'student', 'STU', NULL, '2025ICT5IT01', '03ac674216f3e15c761ee1a5e255f067953623c8b388b4459e13f978d7c846f4', '2025ict5it01@slgti.com', 1, 0, '685c501f17e9367b82b9c0441de25ae0bef7e41d50a656e5d9721c3dd790b88a', NULL, NULL, NULL, 0, NULL, NULL, '7e040fcbc90827d9f7ad184248f9b6b608931314', 1754887873),
(5, 'staff', 'ADM', NULL, 'admin', '$2y$10$Vd8.zUwoySjrq9KoMLYVlehtrF7ufkMeY2a0TertTwC4cPdVboyJu', 'admin@slgti.com', 1, 0, 'ac581abead91f5418db79c64a501c7f3e6df054786ad8466d3070eeb858ac965', NULL, NULL, NULL, 0, NULL, NULL, '292e1e623c3535ddaec88bf9d28c0dc698c4fc59', 1586058126),
...
(80, 'student', 'STU', NULL, 'keerththeejan', '$2y$10$.YBiqpXtHQi/D2TWLVzlful02HAitVLHt8kJ/zcmadzwSYtVDVex.', 'keerththeejan@slgti.lk', 1, 0, '970e0c1226eb422cfa7a8f1518f908de4203c7fcc4c039a42f6cb6da42f70b2b', 1754901344, NULL, NULL, 0, NULL, NULL, NULL, NULL),
(95, 'student', NULL, NULL, 'new', '$2y$10$IRIznPUUINnCGPUqKJTYY./YW2B8udLKT4kghMaA3MDJ462LGZOFG', 'new@slgti.lk', 1, 0, NULL, 1754993987, NULL, NULL, 0, NULL, NULL, NULL, NULL);
...
INSERT INTO `user_permissions` (`id`, `username`, `permission_name`, `is_allowed`, `created_at`, `updated_at`) VALUES
(1, 'student1', 'view_profile', 1, '2025-08-11 16:39:32', '2025-08-11 16:39:32'),
(2, 'student1', 'edit_profile', 1, '2025-08-11 16:39:32', '2025-08-11 16:39:32'),
(3, 'student1', 'email_notifications', 1, '2025-08-11 16:39:32', '2025-08-11 16:39:32'),
(4, 'student1', 'enroll_modules', 1, '2025-08-11 16:39:32', '2025-08-11 16:39:32'),
(5, 'student1', 'view_grades', 1, '2025-08-11 16:39:32', '2025-08-11 16:39:32'),
(10, 'keerththi', 'viewProfile', 1, '2025-08-12 05:17:51', '2025-08-12 05:17:51'),
(11, 'keerththi', 'editProfile', 1, '2025-08-12 05:17:51', '2025-08-12 05:17:51'),
(12, 'keerththi', 'emailNotifications', 1, '2025-08-12 05:17:51', '2025-08-12 05:17:51'),
(13, 'keerththi', 'enrollModules', 1, '2025-08-12 05:17:51', '2025-08-12 05:17:51'),
(14, 'keerththi', 'viewGrades', 1, '2025-08-12 05:17:51', '2025-08-12 05:17:51'),
(15, 'keerththi', 'manageUsers', 1, '2025-08-12 05:17:51', '2025-08-12 05:17:51'),
(16, 'keerththi', 'manageContent', 1, '2025-08-12 05:17:51', '2025-08-12 05:17:51'),
(17, 'keerththi', 'viewReports', 1, '2025-08-12 05:17:51', '2025-08-12 05:17:51'),
(18, 'keerththeejan', 'editProfile', 1, '2025-08-12 09:54:07', '2025-08-12 09:54:07'),
(19, 'keerththeejan', 'emailNotifications', 1, '2025-08-12 09:54:07', '2025-08-12 09:54:07'),
(20, 'keerththeejan', 'enrollModules', 1, '2025-08-12 09:54:07', '2025-08-12 09:54:18'),
(21, 'keerththeejan', 'viewGrades', 1, '2025-08-12 09:54:07', '2025-08-12 09:54:07'),
(22, 'keerththeejan', 'manageUsers', 0, '2025-08-12 09:54:07', '2025-08-12 09:54:07'),
(23, 'keerththeejan', 'manageContent', 0, '2025-08-12 09:54:07', '2025-08-12 09:54:07'),
(24, 'keerththeejan', 'viewReports', 0, '2025-08-12 09:54:07', '2025-08-12 09:54:07'),
(32, 'admin', 'editProfile', 1, '2025-08-12 09:55:46', '2025-08-12 09:55:46'),
(33, 'admin', 'emailNotifications', 1, '2025-08-12 09:55:46', '2025-08-12 09:55:46'),
(34, 'admin', 'enrollModules', 1, '2025-08-12 09:55:46', '2025-08-12 09:55:46'),
(35, 'admin', 'viewGrades', 1, '2025-08-12 09:55:46', '2025-08-12 09:55:46'),
(36, 'admin', 'manageUsers', 0, '2025-08-12 09:55:46', '2025-08-12 09:55:46'),
(37, 'admin', 'manageContent', 0, '2025-08-12 09:55:46', '2025-08-12 09:55:46'),
(38, 'admin', 'viewReports', 0, '2025-08-12 09:55:46', '2025-08-12 09:55:46'),
(39, 'Accountant', 'editProfile', 1, '2025-08-12 09:58:50', '2025-08-12 09:58:50'),
(40, 'Accountant', 'emailNotifications', 1, '2025-08-12 09:58:50', '2025-08-12 09:58:50'),
(41, 'Accountant', 'enrollModules', 0, '2025-08-12 09:58:50', '2025-08-12 09:58:50'),
(42, 'Accountant', 'viewGrades', 1, '2025-08-12 09:58:50', '2025-08-12 09:58:50'),
(43, 'Accountant', 'manageUsers', 0, '2025-08-12 09:58:50', '2025-08-12 09:58:50'),
(44, 'Accountant', 'manageContent', 0, '2025-08-12 09:58:50', '2025-08-12 09:58:50'),
(45, 'Accountant', 'viewReports', 0, '2025-08-12 09:58:50', '2025-08-12 09:58:50');

-- --------------------------------------------------------

--
-- Structure for view `student_surveys`
--
DROP TABLE IF EXISTS `student_surveys`;

DROP VIEW IF EXISTS `student_surveys`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `student_surveys`  AS SELECT `se`.`student_id` AS `student_id`, `se`.`course_id` AS `course_id`, `m`.`module_id` AS `module_id`, `m`.`module_name` AS `module_name`, `fs`.`survey_id` AS `survey_id`, `fs`.`start_date` AS `start_date`, `fs`.`end_date` AS `end_date` FROM (((`student_enroll` `se` join `module` `m` on((`se`.`course_id` = `m`.`course_id`))) left join `feedback_survey` `fs` on(((`m`.`course_id` = `fs`.`course_id`) and (`m`.`module_id` = `fs`.`module_id`) and (curdate() between `fs`.`start_date` and `fs`.`end_date`)))) left join `feedback_done` `fd` on(((`fs`.`survey_id` = `fd`.`survey_id`) and (`se`.`student_id` = `fd`.`student_id`)))) WHERE ((`se`.`student_enroll_status` = 'Following') AND (`fd`.`survey_id` is null)) ;
