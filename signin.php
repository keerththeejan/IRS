...
if (isset($_POST['SignIn']) && !empty($_POST['username']) && !empty($_POST['password'])) {
    ...
    $sql = "SELECT * FROM `user` WHERE `user_name`='$username' AND `user_active`=1";
    $result = mysqli_query($con, $sql);
    
    if (mysqli_num_rows($result) == 1) {
        $row = mysqli_fetch_assoc($result);
        
        // Verify the password - support both old (SHA-256) and new (bcrypt) hashes
        $stored_hash = $row['user_password_hash'];
        $is_legacy_hash = (strlen($stored_hash) === 64 && ctype_xdigit($stored_hash)); // Check if it's a legacy SHA-256 hash
        
        $password_matched = false;
        
        if ($is_legacy_hash) {
            // Check against legacy SHA-256 hash
            $password_matched = (hash('sha256', $password) === $stored_hash);
            
            // If password matches and it's a legacy hash, update to bcrypt
            if ($password_matched) {
                $new_hash = password_hash($password, PASSWORD_DEFAULT);
                $update_sql = "UPDATE `user` SET `user_password_hash` = ? WHERE `user_name` = ?";
            ...
            $password_matched = password_verify($password, $stored_hash);
        }
        ...
        if ($password_matched) {
            ...
            if (!empty($_POST['rememberme'])) {
                ...
                $cookie_string_first_part = $username . ':' . $random_token_string;
                $cookie_string_hash = hash('sha256', $cookie_string_first_part . COOKIE_SECRET_KEY);
                $cookie_string = $cookie_string_first_part . ':' . $cookie_string_hash;
                setcookie('rememberme', $cookie_string, time() + COOKIE_RUNTIME, "/", COOKIE_DOMAIN);
            }
            ...
            if ($row['user_table'] == 'staff') {
                ...
                $sql_s = "SELECT `course`.`department_id` AS `department_id` 
                         FROM `student_enroll` 
                         LEFT JOIN `course` ON `student_enroll`.`course_id` = `course`.`course_id` 
                         WHERE `student_enroll`.`student_id` = '$username'";
                $result_s = mysqli_query($con, $sql_s);
                if (mysqli_num_rows($result_s) == 1) {
                    $row_s = mysqli_fetch_assoc($result_s);
                    $_SESSION['department_code'] = $row_s['department_id'];
            ...
            error_log("Failed login attempt for user: " . htmlspecialchars($username, ENT_QUOTES, 'UTF-8'));
        }
    } else {
        $msg = 'User not found or account is not active. Please contact support if you believe this is an error.';
    }
}
?>
