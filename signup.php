...
if (isset($_POST['SignUp']) && !empty($_POST['username'])) {
  ...
  $sql = "SELECT user_id, user_email, user_active, user_password_hash FROM user WHERE user_name = '$user_name' ";
  $result = mysqli_query($con,$sql);
  if(mysqli_num_rows($result)==1){
    $row = mysqli_fetch_assoc($result);
    $user_email = $row['user_email'];
    $user_active = $row['user_active'];
    $has_password = !empty($row['user_password_hash']);
    
    // If user is inactive but has no password, activate them immediately
    if($user_active==0 && !$has_password) {
        // Generate a random password
        $new_password = generateRandomPassword(10);
        $password_hash = password_hash($new_password, PASSWORD_DEFAULT);
        
        // Activate user and set password
        $sql = "UPDATE user SET user_active = 1, user_password_hash = '$password_hash', 
                user_activation_hash = NULL, user_password_reset_timestamp = NULL
                WHERE user_name = '$user_name'";
                
        if(mysqli_query($con,$sql)){
            $msgs = 'User activated successfully!<br>Username: ' . htmlspecialchars($user_name) . 
                   '<br>Password: <strong>' . htmlspecialchars($new_password) . '</strong>' .
                   '<br><div class="alert alert-warning mt-2">Please save this password. It cannot be recovered.</div>';
        } else {
            $msg = 'Error activating user';
        }
    }
    // If user is inactive but has a password, send activation email
    else if($user_active==0 && $has_password) {
